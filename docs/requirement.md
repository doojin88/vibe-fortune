...
- 현재 구독 상태 표시 (무료/Pro)
- 남은 분석 횟수 표시
- 과거 분석 이력 목록 (날짜, 이름, 미리보기)
- "새 분석하기" 버튼
- "구독 관리" 링크

**사용자 인터랙션**:
- 새 분석하기 버튼 클릭 → 새 분석 페이지로 이동
- 분석 이력 항목 클릭 → 해당 분석 상세 페이지로 이동
- 구독 관리 링크 클릭 → 구독 관리 페이지로 이동
- 남은 횟수 0회 시 → "새 분석하기" 버튼 비활성화 및 구독 안내 메시지 표시
...
- 분석 시작 버튼 클릭 → 로딩 표시 후 AI 분석 진행
- 분석 완료 → 분석 상세 페이지로 자동 이동
- 남은 횟수 0회 시 → 폼 비활성화 및 구독 안내

### 2.4 분석 상세
**접근 권한**: 로그인 필요 (본인 분석만 조회 가능)

**화면 구성**:
- 분석 대상 정보 (이름, 생년월일, 출생시간, 성별)
- 분석 결과 (마크다운 렌더링)
  - 천간과 지지
  - 오행 분석
  - 대운과 세운
